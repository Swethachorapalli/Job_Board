...
const MyJobs = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`http://localhost:3000/myJobs/swethachorapalli@gmail.com`)
      .then(res => res.json())
      .then(data => {
        setJobs(data);
  ...
  const handleSearch = () => {
    // The filtering is done in the useEffect hook
  };
                    ...
                    <tbody>
                      {currentJobs.map((job, index) => (
                        <tr key={job._id}>
                          ...
                          <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                            <Link to={`/edit-job/${job._id}`} className="text-blue-500 hover:text-blue-700">Edit</Link>
                          </td>
                          <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
